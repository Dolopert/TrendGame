# Game Radar

จับเกม Steam ที่กำลังพุ่ง แล้วคัดเฉพาะตัวที่เอาไปปล่อยเช่าไอดีได้ ออกมาเป็นหน้า dashboard

**หน้า dashboard สด:** https://dolopert.github.io/TrendGame/docs/index.html

คำศัพท์ที่ใช้ทั้งโปรเจกต์อยู่ที่ [CONTEXT.md](./CONTEXT.md) · สถานะปัจจุบัน เหตุผลเบื้องหลังการตัดสินใจ และกับดักที่เคยเจอ อยู่ที่ [UPDATE.md](./UPDATE.md) — **อ่านสองไฟล์นั้นก่อนแก้โค้ด**

เครื่องมือนี้ตอบคำถามเดียว: **"ควรเอาเงินไปซื้อไอดีเกมไหนก่อนที่ร้านอื่นจะซื้อ"**
ไม่ใช่ "เกมไหนกำลังดัง" — เกมที่ดังแล้วและคู่แข่งสต็อกไว้ 50 ไอดี มีค่าเป็นศูนย์สำหรับร้านที่เพิ่งเข้าตลาด

## ใช้ยังไง

รันวันละครั้ง (เก็บข้อมูล + สร้าง dashboard ในคำสั่งเดียว):

```bash
uv run --project game-radar game-radar run
```

## รันเองอัตโนมัติบน GitHub แล้ว

**ไม่ต้องสั่งอะไรรายวัน** — [workflow](.github/workflows/daily.yml) รัน **ทุก 6 ชั่วโมง**
(03:00 · 09:00 · 15:00 · 21:00 เวลาไทย) บนเครื่องของ GitHub ทำครบทั้ง `scan` → `dash`
แล้ว commit ผลกลับเข้ารีโป หน้าเว็บจึงอัปเดตเองโดยไม่ต้องเปิดเครื่องที่บ้าน

ที่รันถี่กว่าวันละครั้งเพราะจุดบนกราฟมาจากรอบ `scan` — วันละรอบเดียวคือกราฟ
ได้จุดใหม่วันละจุด มองไม่ออกว่าเกมไหนพุ่งภายในวัน

สั่งรันเดี๋ยวนี้ได้ที่แท็บ Actions → เลือก workflow → Run workflow

### ข้อจำกัดตอนนี้: ข้อมูลตลาดเก็บบน cloud ไม่ได้

หน้าร้าน 499k อยู่หลัง Cloudflare ซึ่งกัน IP ของ datacenter — ยิงจากเครื่อง GitHub
ได้ `HTTP 403` พร้อมหน้า challenge (`<title>Just a moment...`) ส่วนยิงจากเน็ตบ้านผ่านปกติ

**จะไม่แก้ด้วยการหลบ challenge** และ **API ทางการที่
[`/docs/api`](https://store.499k-network.com/docs/api) ก็ใช้ไม่ได้กับกรณีนี้** —
มันเป็น API ของผู้ซื้อ/รีเซลเลอร์ที่เอาสินค้าของ 499k ไปขายต่อบนเว็บตัวเอง
(สั่งซื้อแล้วตัดเงินจากยอดในบัญชี) ไม่ใช่ API สำหรับร้านที่ตั้งอยู่บนแพลตฟอร์มอยู่แล้ว

ทดสอบด้วยคีย์ sandbox แล้ว: `GET /api/v1/products/{id}/availability` คืน `busy[]`
(ช่วงเวลาที่ไอดีถูกจองจริง) กับ `rates` ซึ่งเป็นข้อมูลที่ดีกว่าที่เราเก็บอยู่มาก
แต่คีย์จริงต้องเติมเงินสะสมครบ 2,000 และ **ยังไม่รู้ว่าจะเห็นสต็อกของทุกร้าน
หรือแค่สินค้าที่เปิดให้ซื้อผ่าน API** — ควรถามฝ่ายสนับสนุนก่อนตัดสินใจจ่าย

**จึงแบ่งหน้าที่แทน** — cloud เก็บ Steam / เครื่องที่บ้านเก็บตลาดแล้ว push ขึ้นรีโป
ตั้งอัตโนมัติด้วย [`update_market.ps1`](./update_market.ps1) ผ่าน Task Scheduler ทุกวัน **22:00**
(คนละเวลากับรอบ cloud กันเขียนไฟล์เดียวกันชนกัน)

```powershell
powershell -ExecutionPolicy Bypass -File game-radar/setup_scheduler.ps1
```

สคริปต์ทำตามลำดับนี้ และ **ห้ามสลับ**:

```
pull -> restore --force -> market -> dump -> commit -> push
```

ขั้น `restore --force` คือหัวใจ เพราะต้องเอาข้อมูลที่ cloud เก็บไว้มาเป็นฐานก่อน
ถ้าข้ามไป ตอน dump จะเขียนทับด้วยฐานข้อมูลเก่าในเครื่อง แล้วข้อมูลของ cloud หายทันที

### ต้องเปิดคอมไหม

| ข้อมูล | รันที่ไหน | ต้องเปิดคอม |
|---|---|---|
| Steam + สร้างหน้าเว็บ | GitHub · ทุก 6 ชม. | **ไม่ต้อง** |
| สต็อกตลาดเช่า | เครื่องบ้าน · 22:00 | ต้อง แต่เลื่อนได้ |

ตัว task ตั้ง `StartWhenAvailable` ไว้ ถ้า 4 ทุ่มเครื่องปิดอยู่ Windows จะรันให้
ทันทีที่เปิดเครื่องครั้งถัดไป และตั้ง `WakeToRun` ไว้ด้วย เครื่องที่ **หลับ** อยู่
จะถูกปลุกมารันตามเวลา

ข้อจำกัดที่แก้ไม่ได้:

- **ปลุกจากเครื่องที่ปิดสนิทไม่ได้** WakeToRun ปลุกได้เฉพาะ sleep/hibernate
- **บนแบตเตอรี่จะไม่ปลุก** — power plan ของเครื่องนี้ตั้ง `อนุญาตตัวจับเวลาปลุก`
  ไว้เป็นเปิดเฉพาะตอนเสียบปลั๊ก (AC) ส่วนตอนใช้แบต (DC) ปิดอยู่
  เช็คเองได้ด้วย `powercfg /query SCHEME_CURRENT SUB_SLEEP bd3b718a-0680-4d9d-8ab2-e1d2b4ac806d`
- **ปิดเครื่องหลายวันติดแล้วเปิดมา จะรันแค่รอบเดียว** ไม่ชดเชยย้อนหลังทุกวันที่ขาด
  วันที่ขาดคือหายถาวร เพราะ API ให้แต่ค่า ณ ปัจจุบัน ไม่มีย้อนหลัง

ในทางปฏิบัติ: เปิดเครื่องอย่างน้อยวันละครั้งตอนไหนก็ได้ ข้อมูลก็ไม่ขาด

### กับดักของไฟล์ .ps1 บนเครื่องนี้

เจอมาแล้วทั้งสองแบบ เสียเวลาไปหลายรอบ:

- **ต้องเป็น UTF-8 พร้อม BOM** ไม่งั้น PowerShell 5.1 อ่านภาษาไทยเป็น cp874 แล้ว parser พัง
- **อย่าใช้ backtick ขึ้นบรรทัดใหม่** ถ้าไฟล์เป็น CRLF การต่อบรรทัดจะพังเงียบ ๆ
  (backtick ไปหนี CR แทนที่จะหนีการขึ้นบรรทัด) ใช้ splatting `@{...}` แทน

### ฐานข้อมูลเก็บยังไง

ประวัติที่สะสมมาคือของชิ้นเดียวที่มีค่าจริงในโปรเจกต์นี้ เพราะเก็บย้อนหลังใหม่ไม่ได้
จึง commit ไว้ในรีโปเป็น **`data/radar.sql`** (ไฟล์ข้อความ) ไม่ใช่ `.sqlite3` (ไฟล์ไบนารี)
เพราะ git เก็บไฟล์ไบนารีเป็นสำเนาเต็มทุก commit — ปีเดียวก็บวมหลายร้อย MB
ส่วนไฟล์ข้อความที่เพิ่มบรรทัดท้าย ๆ git ทำ delta ได้ดีมาก

```bash
uv run --project game-radar game-radar restore   # สร้าง .sqlite3 จาก .sql
```

```bash
uv run --project game-radar game-radar dump      # เขียน .sql กลับจาก .sqlite3
```

ดึงรีโปมาเครื่องใหม่แล้วสั่ง `restore` ก็ได้ประวัติครบ (ทดสอบ round-trip แล้วตรงทุกแถว)

**สองฝั่งเขียน `data/radar.sql` ไฟล์เดียวกัน** จึงตั้งเวลาให้ไม่ชนกัน (cloud ทุก 6 ชั่วโมง /
เครื่องบ้าน 22:00) และทั้งคู่ `git pull --rebase` ก่อน push เสมอ ถ้าจะเพิ่มงานเก็บข้อมูล
ที่ไหนอีก ต้องรักษาสองข้อนี้ไว้ ไม่งั้นข้อมูลจะทับกันหาย

แยกเป็นขั้น ๆ ถ้าต้องการ:

```bash
uv run --project game-radar game-radar scan
```

```bash
uv run --project game-radar game-radar dash
```

ดูอันดับเร็ว ๆ ในเทอร์มินัลโดยไม่ต้องเปิดเบราว์เซอร์:

```bash
uv run --project game-radar game-radar top -n 15
```

เปิด dashboard: ใช้ preview config ชื่อ `game-radar` (พอร์ต 5501) หรือเปิดไฟล์
`out/dashboard.html` ตรง ๆ ก็ได้

## หน้า dashboard มีอะไร

- **กราฟดัชนีผู้เล่น** — เส้นของ 8 เกมที่คนเล่นเยอะสุด เทียบจุดแรกที่เก็บได้ = 100
  ใช้ดัชนีแทนค่าดิบเพราะเกมใหญ่กับเกมเล็กต่างกันหลักแสน ถ้าพล็อตค่าดิบเส้นของเกมเล็ก
  จะแบนติดพื้นจนมองไม่เห็นว่ากำลังพุ่ง **เส้นที่โดดขึ้นคือเกมที่กำลังมา**
- **มุมมองการ์ด** — เรียงตามคะแนนน่าซื้อ พร้อมตัวกรอง
- **มุมมองอันดับผู้เล่น** — เรียงตามคนออนไลน์ล้วน ๆ 1, 2, 3, ... ไม่เอาคะแนนมากรอง
  (ไม่งั้นเกมอย่าง CS2 ที่คนเล่นล้านคนแต่เป็น F2P จะหายไปจากอันดับทั้งที่มันอยู่ที่ 1 จริง)
- **เอาเมาส์ชี้ที่คะแนนน่าซื้อ** จะกางให้ดูว่าคะแนนมาจากตัวคูณอะไรบ้าง ตัวไหนเพิ่ม ตัวไหนหัก
- **คลิกเกม** เปิดหน้าร้าน Steam ของเกมนั้น
- **ชั่วโมงเล่นกับคะแนนรีวิว** — ค่ากลางของ `playtime_at_review` จากรีวิวล่าสุด 100 อัน
  บอกว่าควรตั้งแพ็กเช่ากี่วัน (แพลตฟอร์มขายเป็น 1/3/7 วัน) วัดจริงแล้วแยกกลุ่มชัด:
  How to Fish 6 ชม. · R.E.P.O. 31 ชม. · Terraria 99 ชม.
  **ยังไม่มีผลกับคะแนนน่าซื้อ** เพราะยังไม่รู้ว่าเกมยาวเช่าออกดีกว่าหรือแย่กว่าเกมสั้น
  (Terraria 99 ชม. ตลาดสต็อก 24 ใบ ส่วน How to Fish 6 ชม. สต็อก 15 ใบ)
  รอข้อมูลตลาดสะสมพอแล้วค่อยตัดสินด้วยตัวเลข ไม่ใช่เดา

### อัปเดตหน้าเว็บที่ deploy ไว้

หน้าที่ https://dolopert.github.io/TrendGame/docs/index.html มาจาก `docs/index.html` ซึ่ง commit ไว้ในรีโป
GitHub Pages ตั้ง source เป็น branch `main` โฟลเดอร์ `/ (root)` ทั้งรีโปจึงถูก publish
หน้า dashboard เลยอยู่ที่ path `/docs/index.html` ไม่ใช่ที่รากของเว็บ อัปเดตด้วยการสร้างทับแล้วพุช:

```bash
uv run --project game-radar game-radar scan
```

```bash
uv run --project game-radar game-radar dash --out game-radar/docs/index.html
```

ข้อควรรู้: `docs/index.html` เป็นไฟล์ที่ **สร้างขึ้นมา** ไม่ใช่ไฟล์ที่แก้ด้วยมือ
แก้หน้าตาให้ไปแก้ template ใน `src/game_radar/render.py` แทน

## ระบบทำงานยังไง

มีสองสายข้อมูลเข้า: **สัญญาณจาก Steam** (เกมกำลังมาไหม) กับ **สัญญาณจากตลาดเช่าจริง**
(ตลาดรู้แล้วหรือยัง) — สายที่สองคือสิ่งที่ v1 ไม่มี และเป็นเหตุผลที่ v1 ทำนายไม่ได้

Surge กับแกนธุรกิจยังคำนวณแยกกันเหมือนเดิม เพื่อให้ดีบักได้ว่าพังตรงไหน
แต่ตอนนี้มาบรรจบที่ **Opportunity** ก่อนถึงหน้าเว็บ

```mermaid
flowchart TD
    Start(["game-radar run"]) --> C1
    MStart(["game-radar market"]) --> M1

    subgraph C ["① เก็บสัญญาณจาก Steam · scan.py"]
        direction TB
        C1["ชาร์ต + หน้าร้าน<br>3 request"]
        C2["<b>ค้นเกม co-op 3 ถัง</b><br>ขายดี · กำลังมา · เพิ่งออก"]
        C1 --> C2 --> C3["เขียน Snapshot<br>taken_at เดียวกันทั้งชุด"]
        C3 --> C4["เติม metadata ที่เก่า<br>ราคา หมวด <b>วันวางขาย</b>"]
    end

    subgraph M ["①ข เก็บสัญญาณจากตลาด · market.py"]
        direction TB
        M1["สต็อกของทุกร้านบนแพลตฟอร์ม"]
        M1 --> M2["สต็อกของร้านเราเอง"]
    end

    C4 --> DB[("SQLite")]
    M2 --> DB

    DB --> A1
    DB --> B1

    subgraph A ["② ก แกนกระแส"]
        A1{"มีประวัติครบ 3 รอบ?"}
        A1 -- "ครบ" --> A2["ฐาน history"]
        A1 -- "ไม่ครบ" --> A3["ฐาน weekly_rank"]
        A2 --> A4["<b>Surge</b>"]
        A3 --> A4
    end

    subgraph B ["② ข แกนธุรกิจ"]
        B1["สถานะ: prospect /<br>upcoming / blocked"]
        B1 --> B2["<b>Freshness</b><br>ยังไม่ขาย · &lt;1 ปี · 1-2 ปี · เก่า"]
        B2 --> B3["<b>สต็อกตลาด</b><br>คู่แข่งมีกี่ใบ · เรามีกี่ใบ"]
    end

    A4 --> O
    B3 --> O
    O["<b>Opportunity</b><br>กระแส × ความสด × co-op × ช่องว่างที่เหลือ"]
    O --> R1

    subgraph R ["③ แสดงผล · render.py"]
        R1["การ์ด + ตัวกรอง<br>เรียงตามคะแนนน่าซื้อ"]
    end

    R1 --> Out(["docs/index.html → Pages"])
```

### อ่านผังนี้ยังไง

| จุด | ทำไมถึงออกแบบแบบนั้น |
|---|---|
| ชาร์ต 2 ตัวมาก่อน | ได้ CCU และอันดับของ ~150 เกมด้วย 2 request แทนที่จะยิงทีละเกม 150 ครั้ง |
| `taken_at` เดียวกันทั้งชุด | ถ้าแต่ละเกมมีเวลาต่างกัน จะเอา snapshot มาเทียบกันไม่ได้ |
| แคช metadata 3 วัน | `appdetails` ถูก rate limit หนักสุดในบรรดา endpoint ทั้งหมด และราคา/หมวดแทบไม่เปลี่ยนรายวัน |
| แยกสองแกนที่ ② | เหตุผลอยู่ย่อหน้าบนผัง |
| เก็บสต็อกตลาดแยกคำสั่ง | คำสั่ง `market` เบามาก รันได้ทุกวันแม้วันที่ไม่สแกน Steam — ความต่อเนื่องสำคัญกว่าความละเอียด |
| ตัวกรองอยู่ฝั่งเบราว์เซอร์ | หน้าเว็บเป็นไฟล์นิ่ง ๆ ไม่มี server กรองให้ กดแล้วต้องเปลี่ยนทันที |

## ข้อมูลมาจากไหน

ทุก endpoint เป็นของฟรี ไม่ต้องใช้ API key และไม่มีอะไรต้องใส่ `.env`

| endpoint | ได้อะไร | ยิงกี่ครั้งต่อรอบ |
|---|---|---|
| `ISteamChartsService/GetMostPlayedGames` | 100 อันดับ + **อันดับสัปดาห์ก่อน** | 1 |
| `ISteamChartsService/GetGamesByConcurrentPlayers` | CCU สด 100 เกม | 1 |
| `store/api/featuredcategories` | ลดราคา / เกมใหม่ / ขายดี / กำลังมา | 1 |
| `ISteamUserStats/GetNumberOfCurrentPlayers` | CCU รายตัว (เฉพาะเกมนอกชาร์ต) | ~60 |
| `store/api/appdetails` | ราคา หมวดหมู่ แนว ปก วันวางขาย | เท่าจำนวนเกมที่ metadata เก่า |

`appdetails` ถูก Steam จำกัดราว 200 ครั้งต่อ 5 นาที โค้ดจึงหน่วง 1.5 วินาทีต่อครั้ง
และแคชผลไว้ 3 วัน (ตั้งใหม่ได้ที่ `db.stale_appids`) รอบแรกจึงกินเวลาราว 4 นาที
รอบถัด ๆ ไปจะเร็วกว่ามาก

## แบ็คเทสกับตลาดจริง — และสิ่งที่มันหักล้าง

ดึงแค็ตตาล็อกร้านเช่าจริงบน 499k-network (36 เกม 290 ไอดี) มาเทียบกับอันดับที่ v1 ให้:

**Spearman rank correlation = −0.07** คือ **ไม่มีความสัมพันธ์กันเลย** เกมที่ตลาดลงทุนหนักสุด
(R.E.P.O. 50 ไอดี) v1 จัดไว้อันดับ 71 ส่วนเกมที่ v1 จัดอันดับต้น ๆ ตลาดแทบไม่แตะ

สามอย่างที่ v1 เข้าใจผิด และแก้แล้วใน v2:

| v1 คิดว่า | ข้อมูลจริงบอกว่า |
|---|---|
| single-player ปล่อยเช่าง่ายกว่า = ข้อดี | **98% ของไอดีที่ตลาดสต็อกอยู่กับเกมที่มี Multi-player** ส่วน single ล้วนได้ 1% |
| CCU/อันดับ = ตัวชี้วัดดีมานด์ | วัด "ฐานผู้เล่นใหญ่" ซึ่งคนละเรื่องกับ "กลุ่มเพื่อนอยากเล่นด้วยกัน" |
| วันวางขายไม่สำคัญ | **8 ใน 12 เกมที่ตลาดสต็อกหนักสุดออกภายใน 18 เดือน** |
| เกมยังไม่วางขาย = ตัดทิ้ง | มันคือของที่ยังไม่มีใครสต็อกได้เลย = ช่องว่างที่ชัดที่สุด |

สิ่งเดียวที่รอด: ช่วงราคา ฿100-600 — 75% ของแค็ตตาล็อกจริงอยู่ในช่วงนี้ (ค่ากลาง ฿342)

**ข้อควรระวังที่สำคัญที่สุด:** การตรงหรือไม่ตรงกับแค็ตตาล็อกปัจจุบันของคู่แข่ง บอกได้แค่ว่า
*มองถูกหมวดไหม* ไม่ได้บอกว่า *จะเจอเกมถัดไปก่อนเขาไหม* — อันนั้นต้องรอให้ `market` สะสมข้อมูล
ไปข้างหน้าจริงสองสามสัปดาห์ แล้วดูว่าสัญญาณจาก Steam ของเรามาก่อนที่คู่แข่งจะเพิ่มสต็อกหรือเปล่า

## เรื่องที่ต้องเข้าใจก่อนเชื่อตัวเลข

**คะแนนกระแสมีสองเกรด** และ dashboard บอกไว้ตรงการ์ดทุกใบ:

- ช่วง 3 วันแรกยังไม่มีประวัติของตัวเอง คะแนนจึงมาจาก `last_week_rank`
  ที่ Steam แถมมากับชาร์ต — หยาบกว่า แต่มีให้ใช้ตั้งแต่วันแรกเลยไม่ต้องรอ
- ตั้งแต่รอบที่ 4 เป็นต้นไป ระบบสลับไปเทียบกับค่ากลางของตัวเกมเองอัตโนมัติ ซึ่งแม่นกว่ามาก

**"เข้าชาร์ตใหม่" ดูจากอันดับสัปดาห์ก่อนที่เกิน 100** ไม่ใช่จากการที่ไม่มีอันดับ
— ตอนแรกเข้าใจว่า Steam จะไม่ส่ง `last_week_rank` มาให้เกมที่สัปดาห์ก่อนยังไม่ติดชาร์ต
แต่พอยิงจริงพบว่ามันส่งมาเสมอ และส่งค่าที่เกิน 100 ได้ด้วย (เจอ 152 มาแล้ว)
เกมที่กระโดดจากนอกชาร์ตเข้ามาติด 100 อันดับคือสิ่งที่โปรเจกต์นี้ตั้งใจจะจับ

**โหมดผู้เล่นกลับด้านมาแล้วสองครั้ง** รอบแรกออกแบบว่าจะคัดเกมที่มี Multi-player ทิ้ง
ซึ่งจะคัด How to Fish (มีครบทั้ง Single/Multi/Co-op) ทิ้งเอง เลยเปลี่ยนเป็นแค่ป้ายกำกับ
รอบสองพอเทียบกับตลาดจริงพบว่าต้องกลับด้านอีกที — **การมี co-op คือเงื่อนไขเกือบบังคับ**
ไม่ใช่แค่ป้าย เพราะเกมที่เล่นคนเดียวล้วนกินส่วนแบ่งแค่ 1% ของไอดีทั้งตลาด

## ความเสี่ยงของโมเดลธุรกิจ

Steam Subscriber Agreement ไม่อนุญาตให้แชร์หรือให้เช่าบัญชี ไอดีถูกล็อกได้ถ้ามีคนรายงาน
เกมที่มี Online PvP เสี่ยงเพิ่มเพราะระบบ anti-cheat จับ IP ที่กระโดดไปมา
— dashboard ติดหมายเหตุนี้ไว้ที่การ์ดของเกมกลุ่มนั้นแล้ว ส่วนจะรับความเสี่ยงแค่ไหนเป็นการตัดสินใจของคุณ

## โครงสร้าง

```
src/game_radar/
  steam.py    ตัวห่อ Steam API — ไม่มี logic ธุรกิจ
  market.py   ตัวห่อ API ของแพลตฟอร์มร้านเช่า (สต็อกจริง)
  db.py       schema + คำสั่งอ่านเขียน SQLite
  scan.py     การเก็บข้อมูลหนึ่งรอบ (ทั้ง Steam และตลาด)
  score.py    Surge · Freshness · สต็อกตลาด → Opportunity
  render.py   สร้าง dashboard.html
  cli.py      คำสั่ง
data/         ฐานข้อมูล (gitignore)
out/          dashboard ที่สร้างแล้ว (gitignore)
```
